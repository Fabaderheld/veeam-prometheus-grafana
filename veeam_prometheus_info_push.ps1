...
Function Get-UnixTimeUTC([AllowNull()][Nullable[DateTime]] $ttt) {
    if (-Not $ttt) { return 0 }
    [int]$unixtime = (Get-Date -Date $ttt.ToUniversalTime() -UFormat %s).Substring(0, 10)
    return $unixtime
}
...
function Get-NumberOfRestorePoints($JobObject) {
    $Type = $JobObject.info.JobType

    if ($Type -eq 'NasBackup') {
        $NasBackup = Get-VBRNASBackup | Where-Object { $_.JobId -eq $JobObject.Id }
...
"@.Replace("`r`n", "`n")
    ...
    Invoke-RestMethod -Method PUT -Uri "$BASE_URL/metrics/job/veeam_repo_report/instance/$REPO_ID/group/$GROUP/server/$SERVER/name/$REPO_NAME" -Body $body
}
...
$VeeamJobs = @(Get-VBRJob | Sort-Object typetostring, name | Where-Object { $_.BackupPlatform.Platform -ne 'ELinuxPhysical' -and $_.BackupPlatform.Platform -ne 'EEndPoint' })
...
foreach ($Job in $VeeamJobs) {
    ...
    $START_TIME_UTC_EPOCH = Get-UnixTimeUTC($LastSession.progress.StartTimeLocal)
    $STOP_TIME_UTC_EPOCH = Get-UnixTimeUTC($LastSession.progress.StopTimeLocal)
    ...
    if ($Job.Options.JobOptions.RunManually) {
        $LAST_SESSION_RESULT_CODE = 99
    }
    ...
    $SecondsAgo = (Get-UnixTimeUTC(Get-Date)) - $STOP_TIME_UTC_EPOCH
    ...
    $NUMBER_OF_RESTORE_POINTS = Get-NumberOfRestorePoints $Job
...
"@.Replace("`r`n", "`n")
    ...
    Invoke-RestMethod -Method PUT -Uri "$BASE_URL/metrics/job/veeam_job_report/instance/$JOB_ID/group/$GROUP/type/$JOB_TYPE/name/$JOB_NAME/server/$SERVER" -Body $body
}
...
foreach ($Job in $AgentJobs) {
    ...
    $START_TIME_UTC_EPOCH = Get-UnixTimeUTC($LastSession.CreationTime)
    $STOP_TIME_UTC_EPOCH = Get-UnixTimeUTC($LastSession.EndTime)
    ...
    if (-Not $Job.ScheduleEnabled) { $LAST_SESSION_RESULT_CODE = 99 }
    if (-Not $Job.JobEnabled) { $LAST_SESSION_RESULT_CODE = 99 }
    ...
    if ($Job.BackupType -eq 'SelectedFiles') {
        ...
        $RestorePoints = Get-VBRRestorePoint -Backup $AgentBackup | Sort-Object -Property CreationTimeUtc -Descending
        $RestorePointsOnlyFull = $RestorePoints | Where-Object { $_.IsFull }
